...
<div class="wrap">
    ...
    <div class="top-box">
        <h1>📦 재고 히스토리</h1>
        <p>상품을 선택해서 입고 / 출고 / 재고조정 날짜를 조회합니다.</p>
    </div>
    ...
    <div class="card">
        <form action="${pageContext.request.contextPath}/stock/history" method="get" id="historyForm">
        ...
        <table>
            ...
            <c:choose>
                <c:when test="${empty historyList}">
                    <tr>
                        <td colspan="9" class="empty">조회된 히스토리가 없습니다.</td>
                    </tr>
                </c:when>
                ...
                <c:otherwise>
                    <c:forEach var="history" items="${historyList}">
                        <tr>
                            <td>
                                <c:if test="${history.historyType eq 'ADJUST' and history.beforeQty == 0}">
                                    ${history.createdAt}
                                </c:if>
                            </td>
                            ...
                            <td>
                                <c:if test="${history.historyType eq 'IN'}">
                                    ${history.createdAt}
                                </c:if>
                            </td>
                            ...
                            <td>
                                <c:if test="${history.historyType eq 'OUT'}">
                                    ${history.createdAt}
                                </c:if>
                            </td>
                            ...
                            <td>
                                <c:if test="${history.historyType eq 'ADJUST' and history.beforeQty != 0}">
                                    <c:choose>
                                        <c:when test="${history.changeQty > 0}">
                                            <span style="color:#2e7d32; font-weight:bold;">+${history.changeQty}</span>
                                        </c:when>
                                        <c:otherwise>
                                            <span style="color:#c62828; font-weight:bold;">${history.changeQty}</span>
                                        </c:otherwise>
                                    </c:choose>
                                </c:if>
                            </td>

                            <td>${history.afterQty}</td>
                        </tr>
                    </c:forEach>
                </c:otherwise>
            </c:choose>
...
<script>
    ...
    function filterModelOptions(resetModel) {
        ...
        if (type === 'TIRE') {
            color.value = color.value || 'MIDNIGHT';
            hardness.value = hardness.value || 'R';
            resetColorHardnessStyle();
        }

        if (type === 'ARMOUR' || type === 'TUBELESS') {
            color.value = '';
            hardness.value = '';
            lockColorHardness();
        }
    ...
    function applySelectedItemInfo() {
        ...
        modelName.value = selectedOption.dataset.model;
    ...
    function restoreSelectedOption() {
        ...
        modelName.value = selectedModelName;
